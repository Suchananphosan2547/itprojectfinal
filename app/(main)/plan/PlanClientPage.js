...
export default function PlanClientPage() {
  ...
  const handleAddPlan = async (planName) => {
    try {
      ...
      Swal.fire('เกิดข้อผิดพลาด', err.response?.data?.message || 'ล้มเหลวในการเพิ่มแผนงาน', 'error');
      throw err;
    }
  };
  ...
  const handleActivePlan = async (planId) => {
    try {
      ...
      Swal.fire('เกิดข้อผิดพลาด', err.response?.data?.message || 'ล้มเหลวในการเปิดใช้งานแผนงาน', 'error');
    }
  };
  ...
  const handleEditPlan = async (planId, newPlanName) => {
    try {
      ...
      Swal.fire('เกิดข้อผิดพลาด', err.response?.data?.message || 'ล้มเหลวในการแก้ไขแผนงาน', 'error');
      throw err;
    }
  };
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          Swal.fire('เกิดข้อผิดพลาด', err.response?.data?.message || 'ล้มเหลวในการลบแผนงาน', 'error');
        }
      }
    });
  };
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-opacity-50 p-2">
        <div className="bg-white rounded-xl shadow-xl w-full max-w-md flex flex-col max-h-[90vh] plan-section">
          ...
          <div className="flex justify-end p-6 border-t mt-auto flex-shrink-0">
            <button type="button" onClick={onClose} className="bg-slate-200 hover:bg-slate-300 text-slate-800 font-bold py-2 px-4 rounded-lg mr-3">ยกเลิก</button>
            <button type="submit" form="editPlanForm" className="bg-sky-600 hover:bg-sky-700 text-white font-bold py-2 px-4 rounded-lg">บันทึกการแก้ไข</button>
          </div>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen py-4 sm:p-8 font-sans">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center mb-4 gap-2 w-full">
          ...
          {!loading && !error && filteredPlans.length === 0 && <p className="text-center p-6">ไม่พบแผนงาน</p>}
...
<div className="hidden md:block overflow-x-auto">
  <table className="min-w-full bg-white border border-gray-200 rounded-xl shadow-md overflow-hidden">
    <thead className="bg-gray-100">
      <tr>
        {/* 1. รหัสแผนงาน: text-center ภายใน div */}
        ...
        {/* 2. ชื่อแผนงาน: text-center ภายใน div */}
        <th className="px-6 py-3 text-center text-sm font-semibold text-gray-700 uppercase tracking-wider">
          <div className="flex items-center justify-center space-x-1 cursor-pointer hover:text-teal-600 transition">
            <span>ชื่อแผนงาน</span>
          </div>
        </th>
        {/* 3. สถานะแผนงาน: text-center ภายใน div */}
        <th className="px-6 py-3 text-center text-sm font-semibold text-gray-700 uppercase tracking-wider">
          <div className="flex items-center justify-center space-x-1 cursor-pointer hover:text-teal-600 transition">
            <span>สถานะแผนงาน</span>
          </div>
        </th>
        {/* 4. เมนู: text-center ธรรมดา */}
    ...
    <tbody>
      {filteredPlans.map((item, index) => (
        <tr
          key={item.plan_id}
          className={`border-t border-gray-100 hover:bg-teal-50/60 transition duration-150 ${
            index % 2 === 0 ? "bg-white" : "bg-gray-50/30"
          }`}
        >
          {/* 1. รหัสแผนงาน: เพิ่ม text-center */}
          <td className="px-6 py-4 text-gray-800 font-medium text-center">{item.plan_id}</td>
          {/* 2. ชื่อแผนงาน: เพิ่ม text-center */}
          <td className="px-6 py-4 text-gray-700 text-center">{item.plan_name}</td>
          {/* 3. สถานะแผนงาน: ห่อ span ด้วย div ที่มี flex justify-center */}
          ...
          {/* 4. เมนู: เพิ่ม justify-center ใน div ที่ห่อปุ่ม */}
