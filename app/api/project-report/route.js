...
export async function GET(request) {
    ...
    try {
        // 1. ดึง Query Parameters ทั้งหมดจาก URL (page, limit, search, etc.)
        const { searchParams } = new URL(request.url);
        
        const backendUrl = `${EXTERNAL_API_URL}/project-report`;
        ...
        console.error("Proxy GET /project-report Error:", error);
        if (error.response) {
