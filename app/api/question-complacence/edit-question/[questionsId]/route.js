...
export async function PUT(request, { params }) {
    ...
    // 1. ดึง questionsId จาก URL
    const { questionsId } = params; 
    if (!questionsId) {
        return NextResponse.json({ message: "Question ID is required" }, { status: 400 });
    }

    try {
        // 2. อ่านข้อมูล JSON (questions_name, question_type) จาก body
        const body = await request.json();
        
        // 3. สร้าง URL สำหรับเรียก Backend API ให้ถูกต้อง
        const backendUrl = `${EXTERNAL_API_URL}/question-complacence/edit-question/${questionsId}`;
        ...
        console.error(`Proxy PUT /question-complacence/edit-question/${questionsId} Error:`, error);
        if (error.response) {
